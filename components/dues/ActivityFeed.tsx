    ...
    <section className="mt-6">
      ...
      <div className="mt-2 divide-y divide-border rounded-card border border-border bg-surface px-4">
        {items.map((item) => {
          ...
          const row = <div className="py-3 text-[12px]"><div className="flex items-center justify-between gap-3"><span className="font-semibold">{label}</span><span className="font-mono font-bold">₹{item.amountInr.toLocaleString("en-IN")}</span></div><p className="mt-1 text-[10px] text-muted">{item.status ?? item.kind.replace("_", " ")}{item.reversedByPaymentId ? ` · ${PHASE5_UI_COPY.reversed}` : ""}</p></div>;
